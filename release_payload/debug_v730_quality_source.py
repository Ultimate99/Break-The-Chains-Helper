...
ARENA_LEARNING_FILE = PROFILE_DIR / "arena_learning_model.json"
...
# ==============================================================
# V7.3 ARENA V2 — ADAPTIVE BATTLE LEARNING
# ==============================================================
ARENA_LEARNING_LOCK = threading.RLock()

def _arena_learning_default():
    return {
        "schema": 1,
        "settings": {
            "enabled": True,
...
def load_arena_learning_state():
    base = _arena_learning_default()
    try:
        if ARENA_LEARNING_FILE.exists():
            saved = json.loads(ARENA_LEARNING_FILE.read_text(encoding="utf-8"))
            if isinstance(saved, dict):
                if isinstance(saved.get("settings"), dict):
                    base["settings"].update(saved["settings"])
                if isinstance(saved.get("stats"), dict):
                    base["stats"].update(saved["stats"])
                if isinstance(saved.get("buckets"), dict):
...
arena_learning_state = load_arena_learning_state()


def get_arena_learning_state():
    with ARENA_LEARNING_LOCK:
        return json.loads(json.dumps(arena_learning_state))


def save_arena_learning_state():
    with ARENA_LEARNING_LOCK:
        tmp = ARENA_LEARNING_FILE.with_suffix(".tmp")
        tmp.write_text(json.dumps(arena_learning_state, indent=2), encoding="utf-8")
        os.replace(tmp, ARENA_LEARNING_FILE)


def _arena_feature_vector(ref_screen):
    # Compact visual state vector; cheap enough for the live 12 FPS stream.
    try:
        h, w = ref_screen.shape[:2]
        x1 = max(0, int(w * 0.04))
        y1 = max(0, int(h * 0.05))
        x2 = min(w, int(w * 0.98))
        y2 = min(h, int(h * 0.94))
        ...
        pooled = cv2.resize(gray, (8, 6), interpolation=cv2.INTER_AREA).astype(np.float32) / 255.0
        hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
        hist_h = cv2.calcHist([hsv], [0], None, [8], [0, 180]).reshape(-1)
        hist_s = cv2.calcHist([hsv], [1], None, [4], [0, 256]).reshape(-1)
        hist_v = cv2.calcHist([hsv], [2], None, [4], [0, 256]).reshape(-1)
...
def _arena_frame_signature(ref_screen):
    try:
        gray = cv2.cvtColor(ref_screen, cv2.COLOR_BGR2GRAY)
        tiny = cv2.resize(gray, (8, 8), interpolation=cv2.INTER_AREA)
        threshold = float(tiny.mean())
        bits = (tiny >= threshold).reshape(-1)
        value = 0
        for bit in bits:
...
def _arena_signature_distance(a, b):
...
def _arena_time_bucket(elapsed_s):
    return str(max(0, min(8, int(float(elapsed_s or 0.0) // 7.5))))


def _arena_distance(a, b):
...
def arena_predict_loss(vec, elapsed_s, threat=None):
    if not vec:
        return None, "no visual sample"
    state = get_arena_learning_state()
    settings = state.get("settings") or {}
    bucket = (state.get("buckets") or {}).get(_arena_time_bucket(elapsed_s)) or {}
    win = bucket.get("WIN") or {}
    loss = bucket.get("LOSS") or {}
    win_matches = int(win.get("matches") or 0)
    loss_matches = int(loss.get("matches") or 0)
    need = max(1, int(settings.get("min_class_matches") or 3))
    if win_matches < need or loss_matches < need:
        return None, f"warming {win_matches}W/{loss_matches}L (need {need}/{need})"
    d_win = _arena_distance(vec, win.get("centroid") or [])
    d_loss = _arena_distance(vec, loss.get("centroid") or [])
    if d_win is None or d_loss is None:
        return None, "model centroid unavailable"
    denom = max(1e-6, d_win + d_loss)
    learned = max(0.01, min(0.99, d_win / denom))
    if threat is None:
        probability = learned
    else:
        prior = max(0.25, min(0.75, 0.5 + (float(threat) - 50.0) / 200.0))
        probability = learned * 0.86 + prior * 0.14
    return max(0.01, min(0.99, probability)), (
        f"bucket {_arena_time_bucket(elapsed_s)} • {win_matches}W/{loss_matches}L • "
        f"dW={d_win:.3f} dL={d_loss:.3f}"
    )


def _arena_update_centroid(node, vectors):
    vectors = [v for v in vectors if isinstance(v, list) and v]
    if not vectors:
        return
    old_n = int(node.get("samples") or 0)
    centroid = node.get("centroid")
    if not isinstance(centroid, list) or len(centroid) != len(vectors[0]):
    ...
    for vec in vectors:
        ...
        for i, value in enumerate(vec):
            centroid[i] += (float(value) - centroid[i]) / float(n)
    node["samples"] = n
    node["centroid"] = [round(float(x), 6) for x in centroid]


def arena_train_from_trace(result, samples):
    result = str(result or "").upper()
    if result not in ("WIN", "LOSS"):
        return
    grouped = {}
    for sample in samples or []:
        vec = sample.get("v") if isinstance(sample, dict) else None
        if not isinstance(vec, list) or not vec:
            continue
        bucket = _arena_time_bucket(sample.get("t") or 0.0)
        grouped.setdefault(bucket, []).append(vec)
    if not grouped:
        return
    with ARENA_LEARNING_LOCK:
        buckets = arena_learning_state.setdefault("buckets", {})
        for bucket_name, vectors in grouped.items():
            bucket = buckets.setdefault(bucket_name, {})
            node = bucket.setdefault(result, {"matches": 0, "samples": 0, "centroid": []})
            node["matches"] = int(node.get("matches") or 0) + 1
            _arena_update_centroid(node, vectors)
        stats = arena_learning_state.setdefault("stats", {})
        stats["trained_matches"] = int(stats.get("trained_matches") or 0) + 1
        key = "wins" if result == "WIN" else "losses"
        stats[key] = int(stats.get(key) or 0) + 1
    save_arena_learning_state()


def append_arena_learning_trace(record):
    try:
        line = json.dumps(record, ensure_ascii=False, separators=(",", ":"))
        with ARENA_LEARNING_LOCK:
            with ARENA_TRACE_FILE.open("a", encoding="utf-8") as f:
                f.write(line + "\n")
            max_rows = int((arena_learning_state.get("settings") or {}).get("max_trace_matches") or 250)
            if ARENA_TRACE_FILE.stat().st_size > 8_000_000:
                lines = ARENA_TRACE_FILE.read_text(encoding="utf-8", errors="ignore").splitlines()
                ARENA_TRACE_FILE.write_text("\n".join(lines[-max_rows:]) + "\n", encoding="utf-8")
    except Exception:
        pass


def _arena_pilot_context(elapsed_s, loss_probability=None, identity=None, heroes=None):
...
def arena_adaptive_quit_ready():
    ...
    if not settings.get("enabled", True) or not settings.get("adaptive_quit", True):
        return False
    cfg = get_dodge_settings()
    return bool(cfg.get("quit_ref") and cfg.get("confirm_ref"))
...
class ArenaBotEngine:
    def __init__(self, on_update=None, on_log=None):
        ...
        # V7.3 Arena V2 shadow learner / adaptive-loss controller.
        self._learning_trace = None
        self._learning_last_sample_at = 0.0
        self._learning_last_signature = None
        self._learning_last_event_at = 0.0
    ...
    def _validate_pilot_health(self):
        state = get_arena_learning_state()
        settings = state.get("settings") or {}
    ...
    def _perform_dodge(self, ref_screen, actual_w, actual_h, score=0.0, sample_name=None, reason="Owl of Readiness"):
        ...
        self._learning_note_action("QUIT", str(reason or "Smart Dodge"))
    ...
    def _run_strategy_steps(self, now, actual_w, actual_h):
        ...
        for idx, step in enumerate(steps):
            ...
            self._learning_note_action(action, "strategy")
            self._strategy_executed.add(idx)
            with session_lock:
    ...
    # --------------------------
    # V7.3 ARENA V2 — LIVE LEARNING
    # --------------------------
    def _learning_begin(self):
        state = get_arena_learning_state()
        settings = state.get("settings") or {}
        ...
        self._learning_last_report = None
        with session_lock:
            session["current_loss_probability"] = None
            session["current_learning_status"] = "WARMING / OBSERVING"
            session["current_battle_samples"] = 0
        self._health_set("learning", "LEARNING", "Battle observation started")

    def _learning_note_action(self, label, source="bot"):
        trace = self._learning_trace
        if not isinstance(trace, dict) or self.match_started_at is None:
            return
        try:
            elapsed = max(0.0, time.monotonic() - self.match_started_at)
    ...
    def _learning_sample(self, ref_screen, auto_state=None):
        trace = self._learning_trace
        if not isinstance(trace, dict) or self.match_started_at is None:
            return None
        state = get_arena_learning_state()
        ...
        if not settings.get("enabled", True):
            return None
        now_mono = time.monotonic()
        interval = max(0.25, float(settings.get("sample_interval_s") or 0.55))
        if self._learning_last_sample_at and now_mono - self._learning_last_sample_at < interval:
            return None
        self._learning_last_sample_at = now_mono
        elapsed = max(0.0, now_mono - self.match_started_at)
        vec = _arena_feature_vector(ref_screen)
        if not vec:
            return None
        with session_lock:
            threat = session.get("current_threat_score")
        probability, model_detail = arena_predict_loss(vec, elapsed, threat)

        signature = _arena_frame_signature(ref_screen)
        if (
            self._learning_last_signature is not None
            and signature is not None
            and elapsed - self._learning_last_event_at >= 0.28
        ):
            distance = _arena_signature_distance(self._learning_last_signature, signature)
            if distance >= 16:
                events = trace.setdefault("events", [])
                events.append({
                    "t": round(elapsed, 3),
                ...
                self._learning_last_event_at = elapsed
                if len(events) > 240:
                    del events[:-240]
        self._learning_last_signature = signature

        sample = {
            "t": round(elapsed, 3),
            "v": vec,
            "loss_p": round(float(probability), 4) if probability is not None else None,
            "auto": str(auto_state or "?"),
        }
        ...
        if probability is None:
            ...
            status = f"loss {probability * 100.0:.0f}% • {model_detail}"
            health = "ALERT" if probability >= float(settings.get("loss_threshold") or 0.94) else "READY"
        with session_lock:
            session["current_loss_probability"] = (
                round(probability * 100.0, 1) if probability is not None else None
            )
            session["current_learning_status"] = status
            session["current_battle_samples"] = len(samples)
        self._health_set("learning", health, status)

        report_bucket = None if probability is None else int(probability * 10)
        if report_bucket is not None and report_bucket != self._learning_last_report and (
            report_bucket >= 7 or self._learning_last_report is None
        ):
            self._learning_last_report = report_bucket
            self.emit_log(f"ARENA LEARN: {status}")

        min_elapsed = max(0.0, float(settings.get("min_elapsed_s") or 5.0))
        ...
        if probability is not None and elapsed >= min_elapsed and probability >= threshold:
            self._learning_quit_hits += 2 if probability >= 0.985 else 1
        else:
            self._learning_quit_hits = 0
        if self._learning_quit_hits >= confirm_needed:
            return float(probability)
        return None

    def _learning_finish(self, result, was_dodge=False, duration=None):
        trace = self._learning_trace
        ...
        with session_lock:
            trace.update({
                ...
                "opponent_type": session.get("current_opponent_type"),
                "opponent_username": session.get("current_opponent_username"),
                "organization": session.get("current_opponent_org"),
                "identity_confidence": session.get("current_identity_confidence"),
                "detected_heroes": list(session.get("current_detected_heroes") or []),
                "hero_scores": dict(session.get("current_hero_scores") or {}),
                "opponent_power": session.get("current_opponent_power"),
                "threat_score": session.get("current_threat_score"),
                "threat_label": session.get("current_threat_label"),
                "opening_decision": session.get("current_decision"),
                "opening_reason": session.get("current_decision_reason"),
        ...
        if was_dodge:
            with ARENA_LEARNING_LOCK:
                stats = arena_learning_state.setdefault("stats", {})
                stats["dodges"] = int(stats.get("dodges") or 0) + 1
            save_arena_learning_state()
        elif result in ("WIN", "LOSS"):
            arena_train_from_trace(result, trace.get("samples") or [])
        append_arena_learning_trace(trace)
        samples_count = len(trace.get("samples") or [])
        events_count = len(trace.get("events") or [])
        self.emit_log(
            f"ARENA LEARN: stored {result} • {samples_count} state samples • "
            f"{events_count} visual/action events"
        )
        self._learning_trace = None
        self._learning_quit_hits = 0
        self._learning_last_signature = None
        with session_lock:
            session["current_learning_status"] = f"LEARNED FROM {result}"
            session["current_battle_samples"] = samples_count
        ...
        while self.running:
            ...
            try:
                ...
                if auto_state in ("OFF", "ON"):
                    ...
                    # V7.3 Arena V2: continuously learn the live visual state.
                    # Adaptive surrender is only allowed after the model has seen
                    # enough labeled wins and losses and confirms a near-certain loss.
                    adaptive_loss = self._learning_sample(ref, auto_state)
                    if (
                        adaptive_loss is not None
                        and not self.dodge_in_progress
                        and arena_adaptive_quit_ready()
                    ):
                        if self._perform_dodge(
                            ...
                            score=adaptive_loss,
                            sample_name=None,
                            reason=f"Adaptive loss {adaptive_loss * 100.0:.0f}%"
                        ):
                            dodge_checked = True
                    ...
                    if auto_state == "OFF":
                        ...
                        self._learning_note_action("ENABLE_AUTO", "bootstrap")
                        self.emit_log(
...
class App:
    ...
    def _daily_save_failure_frame(self, module, stage, ref, detail=""):
    ...
    def _daily_header_ocr_tokens(self, ref):
    ...
    def _vision_adaptive_state(self):
                    ...
                    if expected:
                        ...
                        if not ok:
                            ...
                            ok,detail,last_after=self._daily_wait_anchor(
                                expected, timeout=2.40, initial_ref=last_after
                            )
